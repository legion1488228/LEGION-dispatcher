...
def manual_composition_data(order_id: int, db: Session = Depends(get_db), user: TelegramUser = Depends(require_admin)):
    ...
    current_brigadier_id = next((a.employee_id for a in assignments if a.role == "brigadier"), None)
    current_member_ids = [a.employee_id for a in assignments if a.role == "member"]
    loads = _workloads(db, order.work_date)
    employees = db.execute(
        select(Employee).where(Employee.active.is_(True)).order_by(Employee.group_code, Employee.full_name)
    ).scalars().all()
    brigadiers = [
...
def owner_cash_ledger(on_date: date, db: Session = Depends(get_db), user: TelegramUser = Depends(require_admin)):
    ...
    entries = db.execute(
        select(CashEntry)
        .where(CashEntry.work_date >= start, CashEntry.work_date <= end)
        .order_by(CashEntry.work_date, CashEntry.brigadier_name, CashEntry.id)
    ).scalars().all()

    current_cashiers = db.execute(
        select(Employee)
        .where(Employee.group_code == "brigadier", Employee.is_cashier.is_(True))
        .order_by(Employee.full_name)
    ).scalars().all()

    brig_by_tg = {
        int(emp.tg_id): emp.full_name
